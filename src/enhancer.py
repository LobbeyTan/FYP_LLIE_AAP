...
class LowLightImageEnhancer:
    ...
    def __preprocessInput(self, image: Image):
        img = self.transform(image)

        self.input_A = self.__getGrayscaleImage(img).unsqueeze(dim=0)
        self.input_A_gray = self.__getAttentionImage(img).unsqueeze(dim=0)

    def __getGrayscaleImage(self, image: torch.Tensor):
    ...
    def __getAttentionImage(self, image: torch.Tensor):
    ...
    def __tensor2im(self, image_tensor, imtype=np.uint8):
    ...
    def process(self, image: Image):
        ...
        output.save(path + "/%s_E.png" % (datetime.now().strftime("%Y%m%d")))
